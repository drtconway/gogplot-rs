        ...
        match column.vtype() {
            VectorType::Int | VectorType::Float => {
                ...
                if valid_values.is_empty() {
                    ...
                    result.insert(format!("{}min", prefix), Box::new(FloatVec(vec![f64::NAN])) as Box<dyn GenericVector>);
                    result.insert(format!("{}max", prefix), Box::new(FloatVec(vec![f64::NAN])) as Box<dyn GenericVector>);
                    result.insert(format!("{}mean", prefix), Box::new(FloatVec(vec![f64::NAN])) as Box<dyn GenericVector>);
                    result.insert(format!("{}median", prefix), Box::new(FloatVec(vec![f64::NAN])) as Box<dyn GenericVector>);
                    result.insert(format!("{}sd", prefix), Box::new(FloatVec(vec![f64::NAN])) as Box<dyn GenericVector>);
                } else {
                    ...
                    result.insert(format!("{}min", prefix), Box::new(FloatVec(vec![min])) as Box<dyn GenericVector>);
                    result.insert(format!("{}max", prefix), Box::new(FloatVec(vec![max])) as Box<dyn GenericVector>);
                    result.insert(format!("{}mean", prefix), Box::new(FloatVec(vec![mean])) as Box<dyn GenericVector>);
                    result.insert(format!("{}median", prefix), Box::new(FloatVec(vec![median])) as Box<dyn GenericVector>);
                    result.insert(format!("{}sd", prefix), Box::new(FloatVec(vec![sd])) as Box<dyn GenericVector>);
                }
            }
            VectorType::Str | VectorType::Bool => {
                ...
                if values.is_empty() {
                    result.insert(format!("{}min", prefix), Box::new(StrVec(vec![String::new()])) as Box<dyn GenericVector>);
                    result.insert(format!("{}max", prefix), Box::new(StrVec(vec![String::new()])) as Box<dyn GenericVector>);
                    result.insert(format!("{}mode", prefix), Box::new(StrVec(vec![String::new()])) as Box<dyn GenericVector>);
                    result.insert(format!("{}n_unique", prefix), Box::new(IntVec(vec![0])) as Box<dyn GenericVector>);
                } else {
                    ...
                    result.insert(format!("{}min", prefix), Box::new(StrVec(vec![min])) as Box<dyn GenericVector>);
                    result.insert(format!("{}max", prefix), Box::new(StrVec(vec![max])) as Box<dyn GenericVector>);
                    result.insert(format!("{}mode", prefix), Box::new(StrVec(vec![mode])) as Box<dyn GenericVector>);
                    result.insert(format!("{}n_unique", prefix), Box::new(IntVec(vec![n_unique])) as Box<dyn GenericVector>);
                }
            }
        }
...
impl StatTransform for Summary {
    fn apply(
        ...
        for aesthetic in &self.aesthetics {
            ...
            let aes_value = mapping.get(aesthetic)
                .ok_or_else(|| PlotError::missing_stat_input("Summary", *aesthetic))?;

            let column_name = aes_value.as_column_name()
                .ok_or_else(|| PlotError::no_valid_data(
                    format!("Aesthetic {:?} must be mapped to a column for Summary stat", aesthetic)
                ))?;

            // Compute summaries for this aesthetic
            let aesthetic_columns = self.compute_for_aesthetic(
                data.as_ref(),
                *aesthetic,
                column_name,
                use_prefix,
...
mod tests {
    ...
    fn test_summary_single_continuous() {
        ...
        let stat = Summary::new(vec![Aesthetic::X]);
        ...
        // Check values
        let mean = output.get("mean").unwrap().iter_float().unwrap().next().unwrap();
        assert!((mean - 3.0).abs() < 1e-10);

        let median = output.get("median").unwrap().iter_float().unwrap().next().unwrap();
    ...
    fn test_summary_multiple_continuous() {
        ...
        let stat = Summary::new(vec![Aesthetic::X, Aesthetic::Y]);
        let result = stat.apply(Box::new(df), &mapping).unwrap();
        ...
        let x_mean = output.get("x_mean").unwrap().iter_float().unwrap().next().unwrap();
        assert!((x_mean - 2.0).abs() < 1e-10);

        let y_mean = output.get("y_mean").unwrap().iter_float().unwrap().next().unwrap();
    ...
    fn test_summary_categorical() {
        let mut df = DataFrame::new();
        df.add_column("group", Box::new(StrVec(vec!["a".to_string(), "b".to_string(), "a".to_string(), "c".to_string()])));
        ...
        let mode = output.get("mode").unwrap().iter_str().unwrap().next().unwrap();
        assert_eq!(mode, "a"); // "a" appears twice

        let n_unique = output.get("n_unique").unwrap().iter_int().unwrap().next().unwrap();

...
impl StatTransform for Density {
    fn apply(
        ...
        if let Some(group_iter) = mapping.get_vector_iter(&Aesthetic::Group, data.as_ref()) {
            ...
            match x_values {
                VectorIter::Int(iter) => {
                    self.compute_grouped(iter, group_iter, &mut new_data, &mut new_mapping)?;
                }
                VectorIter::Float(iter) => {
                    self.compute_grouped(iter, group_iter, &mut new_data, &mut new_mapping)?;
                }
                VectorIter::Str(_) | VectorIter::Bool(_) => {
                    return Err(PlotError::aesthetic_type_mismatch(
                        Aesthetic::X(AestheticDomain::Continuous),
                        "Density stat",
                        "continuous data (integer or float)",
                        "received categorical data (string or boolean)",
                    ));
                }
            }
            ...
            match x_values {
                VectorIter::Int(iter) => {
                    self.compute_ungrouped(iter, &mut new_data, &mut new_mapping)?;
                }
                VectorIter::Float(iter) => {
                    self.compute_ungrouped(iter, &mut new_data, &mut new_mapping)?;
                }
